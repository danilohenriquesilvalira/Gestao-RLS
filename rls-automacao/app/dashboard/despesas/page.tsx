// ✅ PÁGINA DESPESAS CORRIGIDA - SEM LOOPS
'use client';
...
export default function DespesasPage() {
    ...
    const [loadingRef, setLoadingRef] = useState(false); // ✅ Flag para evitar sobreposição
...
>>>>>>> 4cf0e25e8448e95e3242d17810777c3ded133477
    ...
    // ✅ FUNÇÃO COM useCallback E FLAG DE PROTEÇÃO
    const fetchDespesas = useCallback(async () => {
        if (!user?.id || loadingRef) return; // ✅ Proteção contra chamadas simultâneas
        ...
        try {
            ...
            setLoading(false);
            setLoadingRef(false); // ✅ Libera flag
        }
    }, [user?.id, isAdmin]); // ✅ Dependências mínimas e estáveis
    ...
    }, [user?.id, fetchDespesas]); // ✅ fetchDespesas agora é estável

    // ✅ FUNÇÃO DE CRIAR COM useCallback
    const handleCreateDespesa = useCallback(async (e: React.FormEvent) => {
        e.preventDefault();

        if (!user?.id || loadingRef) return;
    ...
    }, [user?.id, novaDespesa, fetchDespesas, loadingRef]);

    // ✅ FUNÇÃO DE STATUS COM useCallback
    const handleStatusChange = useCallback(async (despesaId: number, novoStatus: 'aprovada' | 'rejeitada', observacoes?: string) => {
